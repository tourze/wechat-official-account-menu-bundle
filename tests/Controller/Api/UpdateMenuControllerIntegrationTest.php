...
final class UpdateMenuControllerIntegrationTest extends AbstractWebTestCase
{
    protected function onSetUp(): void
    {
        // 不需要手动清理，AbstractEasyAdminControllerTestCase 会自动处理
        // 集成测试依赖于测试框架的自动数据库管理
    }
    ...
    public function testPutAndDeleteMethodsAreAllowed(): void
    {
        $client = self::createClientWithDatabase();
        ...
        // 测试GET方法不被允许
        try {
            $client->request('GET', '/admin/menu/version/' . $menuVersion->getId() . '/menu/' . $menuButton->getId());
            $response = $client->getResponse();
        ...
        // 测试POST方法不被允许
        try {
            $client->request('POST', '/admin/menu/version/' . $menuVersion->getId() . '/menu/' . $menuButton->getId());
            $response = $client->getResponse();
            $this->assertSame(Response::HTTP_METHOD_NOT_ALLOWED, $response->getStatusCode());
        } catch (MethodNotAllowedHttpException $e) {
            $this->assertInstanceOf(MethodNotAllowedHttpException::class, $e);
    ...
    public function testAuthenticatedUpdateWithInvalidData(): void
    {
        $client = self::createClientWithDatabase();
