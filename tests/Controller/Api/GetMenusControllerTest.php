...
final class GetMenusControllerTest extends AbstractWebTestCase
{
    ...
    public function testGetMenusWithValidVersion(): void
    {
        $client = self::createClientWithDatabase();
        // 使用内存用户避免数据库依赖
        $user = new InMemoryUser('admin', 'password', ['ROLE_ADMIN']);
    ...
    public function testGetMenusWithNonExistentVersion(): void
    {
        $client = self::createClientWithDatabase();
        // 使用内存用户避免数据库依赖
        $user = new InMemoryUser('admin', 'password', ['ROLE_ADMIN']);
